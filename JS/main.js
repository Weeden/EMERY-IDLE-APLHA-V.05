...
function cutWood() {   //woodcutting function
  woodcutting.wood += Math.floor((Math.random() * 3) + 1);
...
function mineRock() {   //mining function
  mining.rock += Math.floor((Math.random() * 3) + 1);
...
function catchFish()  {   //fishing function
  fishing.fish += Math.floor((Math.random() * 3) + 1);
...
var stocksGameLoop = window.setInterval(function() {
  ...
  update("bankCashMade", (bank.bankMoney) + " bitcoin")
}, 10000)
/*
...
var saveGameLoop4 = window.setInterval(function() {
  localStorage.setItem('bankMoneyMadeSave', JSON.stringify(bank))
}, 10000)
